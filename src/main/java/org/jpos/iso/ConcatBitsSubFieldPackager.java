 ...
 */
public class ConcatBitsSubFieldPackager extends CustomSubFieldPackager {

    public ConcatBitsSubFieldPackager() throws ISOException {
        super();
    }
        ...
        try {
            ...
            for (int i = getFirstField(); i <= m.getMaxField(); i++) {
                ...
                if (c != null) {
                    try {
                        if (fld[i] != null) {
                            String field = new String(fld[i].pack(c));
                            sb.append(field);
                        }
